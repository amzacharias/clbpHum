...
# Perform single contrasts between chronotypes
...
# Ensuring variables in design are correctly factorized !!
# Options -----------------------------------------
projDir <- "4_pheno"
...
# Define arguments ==================
optionList <- list(
  make_option(c("-c", "--candPath",
    type = "character", default = NA,
    help = "the path to the gene candidates txt file", metavar = "character"
  )),
...
# Outputs =====
baseDir <- file.path(projDir, "2_edgeRCandidate")
...
candidates <- read.table(opt$candPath, header = FALSE, sep = "\n") %>%
  rename("gene_name" = "V1")
candidatesId2Name <- candidates %>% left_join(id2name)
...
# After dispersion estimation to avoid incorrect results
fit <- fit[rownames(fit) %in% candidatesId2Name$isoform_id, ]

candGenesIncluded <- unique(candidatesId2Name$gene_name[
  match(rownames(fit), candidatesId2Name$isoform_id)
])
candGenesNotIncluded <- unique(
  candidatesId2Name$gene_name[
    match(
      setdiff(candidatesId2Name$isoform_id, rownames(fit)), candidatesId2Name$isoform_id
    )
  ]
)
...
cat(
  ...
  "\nNumber of features after subset:", length(rownames(fit)),
  "\nNumber of unique features after subset:", length(unique(rownames(fit))),
...
for (coeff in names(resList)) {
  if (sum(grepl("logFC", colnames(resList[[coeff]]$topgenesIdName))) == 1) {
    ...
    # PDF
    MakeVolcano(resList[[coeff]]$topgenesIdName, coeff,
      showName = FALSE, newTitle = titles[[coeff]],
    ...
    MakeVolcano(resList[[coeff]]$topgenesIdName, coeff,
      showName = TRUE, newTitle = titles[[coeff]],
      newPath = basePlotsDir,
      sigColor = colors[[coeff]],
      ylims = c(0, maxLim),
      newHeight = 150
